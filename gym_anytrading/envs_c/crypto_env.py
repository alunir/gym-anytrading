...
from gym_anytrading.envs.typedefs import Positions, RewardType
...
class CryptoEnv(TradingEnv):
    ...
    def __init__(
        ...
        super().__init__(df, window_size, render_mode, epsilon=trade_fee)
    ...
    def _calculate_reward(self, action):
        ...
        if (self._position == Positions.Long and action < -self.epsilon) or (
            self._position == Positions.Short and action > self.epsilon
        ):
            ...
            if self._position == Positions.Short:
                ...
                quantity = self._total_profit / last_trade_price * self.leverage
                profit = (
    ...
    def _update_profit(self, action):
        ...
        if (
            (self._position == Positions.Long and action < -self.epsilon)
            or (self._position == Positions.Short and action > self.epsilon)
            or self._truncated
        ):
            current_price = self.prices[self._current_tick]
            last_trade_price = self.prices[self._last_trade_tick]
